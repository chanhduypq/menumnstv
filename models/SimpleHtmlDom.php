...
class SimpleHtmlDom extends Model {
	...
	// load html from file
	//function load_file()				//QV
	function load_file($args)			//QV
	{
		//========================= Quang Vu ==================================== 20160328 ST
		//$args = func_get_args();
		//$this->load(call_user_func_array('file_get_contents', $args), true);
		$file_content = file_get_contents($args);
		//echo "QV <br>" . $file_content; exit;
		$this->load($file_content, true);
		//========================= Quang Vu ==================================== 20160328 ST
	...
	function findDom($selector, $idx=null, $lowercase=false)
	{
		return $this->root->findDom($selector, $idx, $lowercase);
	}
	...
	protected function parse_charset()
	{
		...
		if (empty($charset))
		{
			$el = $this->root->findDom('meta[http-equiv=Content-Type]',0, true);
			if (!empty($el))
			{
	...
	function getElementById($id) {return $this->findDom("#$id", 0);}
	function getElementsById($id, $idx=null) {return $this->findDom("#$id", $idx);}
	function getElementByTagName($name) {return $this->findDom($name, 0);}
	function getElementsByTagName($name, $idx=-1) {return $this->findDom($name, $idx);}
